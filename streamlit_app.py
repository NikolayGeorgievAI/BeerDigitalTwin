...
def build_hop_features(hop_bill_dict, hop_features):
    """
    Take something like {"Citra": 40, "Mosaic": 20, ...}
    and map it into the correct column order expected by the hop model.
    """
    row = []
...
def get_weighted_malt_vector(malt_selections, malt_df, malt_features):
    ...
    We build a feature row that's a % blend of the chosen malts.
    """
...
def get_yeast_feature_vector(yeast_name, yeast_df, yeast_features):
    """
    Pull Temp_avg_C, Flocculation_num, Attenuation_pct for the chosen yeast.
    """
...
def make_radar_chart(hop_profile, hop_top_notes, malt_traits, yeast_traits, style_guess):
    """
    Build a nice 2-column figure:
    - left: radar plot
    - right: flavor/style summary
    """

    labels = list(hop_profile.keys())          # e.g. ["tropical","citrus",...]
    vals   = list(hop_profile.values())        # predicted intensities
    n      = len(labels)
    ...
    gs = fig.add_gridspec(nrows=1, ncols=2, width_ratios=[1.2, 1], wspace=0.4)

    # --- Radar axis ---
    ax = fig.add_subplot(gs[0, 0], polar=True)

    # put first label at top
    ...
    # 2) pick a "nice" max radius so the web isn't microscopic
    max_val = max(max(vals), 1e-6)
    rmax = max_val * 1.2
    if rmax == 0:
        rmax = 1.0  # fallback if literally all zeros
    ax.set_ylim(0, rmax)

    # lighter grid
    ax.grid(color="gray", linestyle="--", linewidth=0.7, alpha=0.6)

    # title on radar side (centered above)
    ax.set_title("Hop Aroma Radar", fontsize=22, fontweight="bold", pad=20)

    # numeric value label for EACH axis point
    for angle, val in zip(angles[:-1], vals):
        # position a tiny bit outward from the actual value so it doesn't sit on the line
        label_radius = val + (rmax * 0.03)
        ax.annotate(
            f"{val:.3f}",
    ...
    # Top hop notes
    text_ax.text(0.0, cursor_y, "Top hop notes:", fontsize=16, fontweight="bold", va="top")
    cursor_y -= 0.07
    if hop_top_notes:
        for note in hop_top_notes:
            text_ax.text(0.03, cursor_y, f"• {note}", fontsize=14, va="top")
            cursor_y -= 0.06
    else:
        text_ax.text(0.03, cursor_y, "(none)", fontsize=14, va="top", style="italic")
        cursor_y -= 0.06

    # Malt character
    cursor_y -= 0.03
    text_ax.text(0.0, cursor_y, "Malt character:", fontsize=16, fontweight="bold", va="top")
    cursor_y -= 0.06
    malt_txt = ", ".join(malt_traits) if malt_traits else "(none detected)"
    text_ax.text(0.03, cursor_y, malt_txt, fontsize=14, va="top")
    cursor_y -= 0.08

    # Yeast character
    text_ax.text(0.0, cursor_y, "Yeast character:", fontsize=16, fontweight="bold", va="top")
    cursor_y -= 0.06
    yeast_txt = ", ".join(yeast_traits) if yeast_traits else "(none detected)"
    text_ax.text(0.03, cursor_y, yeast_txt, fontsize=14, va="top")
    cursor_y -= 0.08

    # Style direction
    text_ax.text(0.0, cursor_y, "Style direction:", fontsize=16, fontweight="bold", va="top")
    cursor_y -= 0.06
    text_ax.text(0.03, cursor_y, f"🍺 {style_guess}", fontsize=14, va="top")
...
for i in range(1, 4 + 1):
    default_index = min(i - 1, len(all_hops) - 1) if len(all_hops) > 0 else 0
    hop_name = st.sidebar.selectbox(f"Hop {i}", all_hops, index=default_index, key=f"hop_{i}_name")
    hop_amt  = st.sidebar.slider(f"{hop_name} (g)", 0, 120, 40, 5, key=f"hop_{i}_amt")
...
malt_selections = []
default_pcts = [70.0, 20.0, 10.0]
for i in range(1, 3 + 1):
    malt_name = st.sidebar.selectbox(f"Malt {i}", malt_options, key=f"malt_{i}_name")
    pct_val   = st.sidebar.number_input(
        f"Malt {i} %",
        min_value=0.0,
        max_value=100.0,
        value=default_pcts[i-1],
        step=1.0,
        key=f"malt_{i}_pct"
    )
    malt_selections.append({"name": malt_name, "pct": pct_val})

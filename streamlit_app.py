...
def get_all_hop_names(hop_features):
    hop_names = [c.replace("hop_", "") for c in hop_features]
    st.sidebar.markdown("**🧬 Model trained on hops:**")
    st.sidebar.write(", ".join(hop_names))
    return hop_names
...
def build_hop_features(hop_bill_dict, hop_features):
    ...
    Warns if none of the selected hops are recognized.
    """
    row = []
    matched = []
    ...
    if not matched:
        st.warning("⚠️ None of your selected hops match the model’s trained hops. "
                   "Try using one of: " + ", ".join([c.replace('hop_', '') for c in hop_features]))
...
def plot_hop_radar(hop_profile, title="Hop Aroma Radar"):
    ...
    for ang, val in zip(base_angles, values_arr):
        ax.text(
            ...
            fontsize=10, bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="#1f77b4", lw=1)
        )
...
malt1_pct  = st.sidebar.number_input("Malt 1 %", 0.0, 100.0, 70.0, step=1.0, key="malt1_pct")
malt2_name = st.sidebar.selectbox("Malt 2", malt_options, key="malt2_name")
malt2_pct  = st.sidebar.number_input("Malt 2 %", 0.0, 100.0, 20.0, step=1.0, key="malt2_pct")
malt3_name = st.sidebar.selectbox("Malt 3", malt_options, key="malt3_name")
malt3_pct  = st.sidebar.number_input("Malt 3 %", 0.0, 100.0, 10.0, step=1.0, key="malt3_pct")
...
if run_button:
    ...
    with col_right:
        ...
        st.write("\n".join([f"- {n}" for n in hop_notes]) if hop_notes else "_No dominant hop note_")
    ...
    st.info("👈 Build your hop bill, malt bill, and yeast — then click **Predict Flavor 🧪**.")
